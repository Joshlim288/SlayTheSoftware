...
namespace Map
{
    /// <summary>
    /// MODIFIED 
    /// Handles the rendering and displaying of the LevelMap
    /// 
    ...
    public class MapView : MonoBehaviour
    {
        ...
        {
            int iterations = 0;
            Level[] currentLevels = null;
            Debug.Log("ShowMap runs");
            Debug.Log("levelId: "+CurrentUser.currentLevelId.ToString());
            ...
            Debug.Log("worldId: "+CurrentUser.currentWorldId.ToString());

            // MODIFICATION: Set current path based on Student's current position
            if (CurrentUser.currentWorldId <= 3)
            {
                if (CurrentUser.hasCompleted) 
                {
                    iterations = 9;
                }
                else 
                {
                    int sectNum;
                    Debug.Log(GameData.currentWorld.sections.Length);
                    for(sectNum = 0; sectNum<GameData.currentWorld.sections.Length ;sectNum++) 
                    {
                        if (GameData.currentWorld.sections[sectNum].id == CurrentUser.currentSectionId)
                        {
                            currentLevels = GameData.currentWorld.sections[sectNum].levels;
                            break;
                        }
                    }
                    ...
                    int levelNum;
                    Debug.Log(sectNum);
                    for (levelNum = 0; levelNum<currentLevels.Length; levelNum++)
                    {
                        if (currentLevels[levelNum].id == CurrentUser.currentLevelId)
                        {
                            iterations = levelNum+3*(sectNum);
                            break;
                        }
                    }
                }
            ...
            {
                if (CurrentUser.hasCompleted) iterations = 4;
                else 
                {
                    currentLevels = GameData.currentWorld.sections[0].levels;
                    for (int i = 0; i<currentLevels.Length; i++)
                    {
                        if (currentLevels[i].id == CurrentUser.currentLevelId)
                        {
                            iterations = i;
                        }
                    }
            ...
            {
                mapManager.CurrentMap.path.Add(mapManager.CurrentMap.nodes[i].point);
            }
